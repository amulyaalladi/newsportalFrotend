...
import { Link, useNavigate } from "react-router"; // Fix 1: Use react-router-dom
import { toast } from "react-toastify";
import { loginUser } from "../../services/authServices";
import { loginSuccess } from "../../redux/authSlice"; // Fix 2: Import loginSuccess
...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Extract user and token properly from response.data
      const user = response?.data?.user || response?.user;
      const token = response?.data?.token || response?.token;

      // Fix 3: Dispatch loginSuccess so user AND token both get saved properly
...
console.log("LOGIN USER OBJECT:", user);
      ...
      // Check role directly on the extracted user object
     if (user?.role === "admin") {
  console.log("Navigating to /admin");
  navigate("/admin");
} else {
  console.log("Navigating to /home, role was:", user?.role);
  navigate("/home");
}
